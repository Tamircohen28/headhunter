...
If the contact email is missing, note: "No recruiter email on file — add via `/jobtrack:contacts` or manually."
...
> "Copy the email above and send it from your email client. Run `/jobtrack:add-task` to set a reminder for the response."

## Step 4 — Create follow-up tasks

For each follow-up sent or skipped, offer:
> "Add a task to check for response in 5 days?" → `/jobtrack:add-task`
