---
name: integrations
description: Connect and sync JobTrack with external tools — Notion, Google Calendar, Google Tasks, Todoist, GitHub, LinkedIn, email/WhatsApp reminders, and CSV import/export. Triggers on connect notion, sync calendar, google tasks, todoist, github, linkedin, export csv, import csv, whatsapp reminder.
...
- **Stale-application reminders — implemented:** ACTIVE_STAGES apps not updated
  ...
  ```
  0 9 * * * node /path/to/jobtrack/scripts/send-stale-reminders.js >> ~/.jobtrack/stale-reminders.log 2>&1
  ```
...
`/jobtrack:brief` uses WebSearch to research:
