...
//        node csv-import.js --url <url> [--write]
...
const file = urlIdx !== -1 ? null : process.argv[2];
const remoteUrl = urlIdx !== -1 ? process.argv[urlIdx + 1] : null;
const write = process.argv.includes("--write");

if (!file && !remoteUrl) {
  console.error("Usage: csv-import.js <input.csv> [--write]\n       csv-import.js --url <url> [--write]");
  process.exit(1);
}
...
(async () => {
  ...
  if (remoteUrl) {
    ...
    if (!res.ok) { console.error(`Failed to fetch CSV from ${remoteUrl}: ${res.status}`); process.exit(1); }
