...
> "Add target roles to your profile first: `/headhunter:setup` → Step 4, or run:
...
- **"Add selected to pipeline"** — user specifies which numbers (e.g. "add 1, 3")
  ...
  ```
- **"Add all top-5"**
- **"Deep scan #N"** → `/headhunter:scan <url>` on the selected job
- **"Get brief for #N"** → `/headhunter:brief <url>`
- **"Skip all"** — no action

After saving, confirm:
> "Added 2 jobs to your pipeline. Run `/headhunter:scan` to score them, or `/headhunter:pipeline` to see the board."
