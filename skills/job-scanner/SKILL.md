...
If the profile is missing or empty (no `experience.key_skills`), tell the user to run `/headhunter:setup` first. A scan without a profile produces meaningless scores.
...
### Recommendation
Apply only if you can invest 3+ hours tailoring. Run /headhunter:apply to generate a
...
- **"Generate tailored CV + cover letter"** → `/headhunter:apply`
- **"Get the full company briefing"** → `/headhunter:brief`
- **"Start interview research pipeline"** → `/headhunter:research`
