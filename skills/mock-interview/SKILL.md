...
> "Run `/headhunter:setup` first — the mock interviewer tailors questions to your background and target role."
...
1. Append the session to `interview.mock_sessions` if an interview record exists:
   ...
   ```
   This lets `/headhunter:insights` correlate mock performance with real interview outcomes.
...
3. Offer to:
   - Add prep tasks for the improvement areas → `/headhunter:add-task`
   - Run another session with a different focus
   - Start the full research pipeline → `/headhunter:research`
